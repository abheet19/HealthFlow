...
## Current candidate check — 2026-09-15
...
Frontend lint and the production build pass with a 151.18 kB gate entry (48.98 kB gzip); after workspace entry, `WorkspaceApp` loads as a separate 155.89 kB chunk (54.06 kB gzip), followed by the selected department route. All 15 backend tests pass: seven credential/realtime cases, four isolated-sample cases, two persisted clinic-query cases, and two DOCX regressions. Current production-only npm audits report zero known vulnerabilities in the frontend and verifier lockfiles, and `pip-audit` reports zero known backend vulnerabilities. The complete frontend dependency tree still reports eight build/lint-only advisories through the Vite 4/ESLint 8 toolchain; those packages are absent from the final Nginx image, and a major toolchain migration remains separate work. Pinned Lighthouse 13.0.1 lab runs against the deployed production build scored 100/100 for Performance, Accessibility, Best Practices, and SEO on mobile and desktop; mobile FCP/LCP were 1.1 s, TBT 0 ms, and CLS 0. Lighthouse is a lab sample and automated accessibility check, not field data or independent WCAG certification.

## Environment and safety

Full mutable workflows run in GitHub's isolated Docker stack: **PostgreSQL 17**, Flask/Gunicorn on **Python 3.12**, and the production Vite bundle served by Nginx. The default local flow uses clinic `demo`, user `demo-user`, and a synthetic-only code; separate backend regressions use two exact synthetic clinic identities. Browser contexts share neither localStorage nor sessionStorage; department changes arrive through real Socket.IO. Public verification uses no deployment secret: it checks the access boundary, revision/health endpoints, and all six routes of the isolated read-only sample while proving that every write control stays disabled and no mutation request leaves the browser.
...
| Photo | Upload, render a thumbnail preview, and delete a synthetic image | Passed |
...
The frontend is a Vite bundle built in a Node container and served by Nginx. `frontend/src/App.tsx` keeps the access boundary small and lazy-loads `WorkspaceApp` only after the tab has authenticated or explicitly entered the isolated sample. `WorkspaceApp` then mounts MUI, routing, the shared patient/socket and toast providers, and lazy department routes behind `React.Suspense`; sample mode seeds fixed data and never opens the realtime connection. The backend image uses Python 3.12, installs explicitly pinned direct dependencies and PostgreSQL client tools, then starts one Gunicorn Eventlet worker through `backend/entrypoint.sh`. PostgreSQL is a separate stateful service.
